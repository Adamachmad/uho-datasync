...
<nav class="navbar" id="navbar">
    <a href="{{ route('home') }}" class="nav-brand">
        <img src="{{ asset('storage/Logo-UHO-Normal-1.png') }}" alt="UHO">
        <div>
    ...
    <div class="nav-actions">
        @if(Auth::guard('pengaju')->check())
            {{-- Sudah Login: Tampilkan account dropdown --}}
            @php $pengajuAuth = Auth::guard('pengaju')->user(); @endphp
            <div class="account-menu">
                <div class="account-trigger">
                    <div class="account-avatar">
                        {{ strtoupper(substr($pengajuAuth->nama_lengkap, 0, 1)) }}
                    </div>
                    <span class="account-name">{{ $pengajuAuth->nama_lengkap }}</span>
                ...
                <div class="account-dropdown">
                    <div class="dropdown-header">
                        <div class="d-name">{{ $pengajuAuth->nama_lengkap }}</div>
                        <div class="d-email">{{ $pengajuAuth->email }}</div>
                    </div>
    ...
    <div class="hero-content">
        <div class="hero-left">
            ...
            <div class="hero-cta">
                ...
                @else
                    <a href="{{ route('daftar') }}" class="cta-primary">
...
<section class="section-layanan" id="layanan">
    <div class="section-inner">
        ...
        <div class="layanan-grid">
            {{-- Card-card layanan --}}
            <a href="{{ Auth::guard('pengaju')->check() ? route('dashboard') : 'javascript:void(0)' }}"
               class="layanan-card"
               onclick="{{ Auth::guard('pengaju')->check() ? '' : "showLoginModal(event, 'Perubahan Nama')" }}">
                @guest('pengaju')
            ...
            <a href="{{ Auth::guard('pengaju')->check() ? route('dashboard') : 'javascript:void(0)' }}"
               class="layanan-card"
               onclick="{{ Auth::guard('pengaju')->check() ? '' : "showLoginModal(event, 'Perubahan Tempat/Tanggal Lahir')" }}">
                @guest('pengaju')
                    <span class="login-required-tag">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect width="18" height="11" x="3" y="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        Login Dulu
            ...
            <a href="{{ Auth::guard('pengaju')->check() ? route('dashboard') : 'javascript:void(0)' }}"
               class="layanan-card"
               onclick="{{ Auth::guard('pengaju')->check() ? '' : "showLoginModal(event, 'Perubahan NIM')" }}">
                @guest('pengaju')
                    <span class="login-required-tag">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect width="18" height="11" x="3" y="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        Login Dulu
            ...
            <a href="{{ Auth::guard('pengaju')->check() ? route('dashboard') : 'javascript:void(0)' }}"
               class="layanan-card"
               onclick="{{ Auth::guard('pengaju')->check() ? '' : "showLoginModal(event, 'Perubahan Jenis Kelamin')" }}">
                @guest('pengaju')
                    <span class="login-required-tag">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect width="18" height="11" x="3" y="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        Login Dulu
            ...
            <a href="{{ Auth::guard('pengaju')->check() ? route('dashboard') : 'javascript:void(0)' }}"
               class="layanan-card"
               onclick="{{ Auth::guard('pengaju')->check() ? '' : "showLoginModal(event, 'Perubahan Agama')" }}">
                @guest('pengaju')
                    <span class="login-required-tag">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect width="18" height="11" x="3" y="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        Login Dulu
            ...
            <a href="{{ Auth::guard('pengaju')->check() ? route('dashboard') : 'javascript:void(0)' }}"
               class="layanan-card"
               onclick="{{ Auth::guard('pengaju')->check() ? '' : "showLoginModal(event, 'Perubahan Lainnya')" }}">
                @guest('pengaju')
                    <span class="login-required-tag">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect width="18" height="11" x="3" y="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        Login Dulu
...
{{-- ===== FOOTER ===== --}}
<footer class="footer">
    <p>© {{ date('Y') }} <strong>UHO-Datasync</strong> — Portal Perubahan Data PDDIKTI Universitas Halu Oleo</p>
...
{{-- ===== LOGIN MODAL (muncul saat menu diklik tapi belum login) ===== --}}
